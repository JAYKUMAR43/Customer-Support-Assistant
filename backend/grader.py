...
class Grader:
    ...
    def evaluate(observation: Observation, action: Action) -> tuple[float, str, float]:
        ...
        # Task 2: Medium
            ...
            if observation.issue_type == IssueType.DAMAGED:
                if action.action_type in [ActionType.REPLACE, ActionType.REFUND]:
                    ...
                    business_impact = -observation.order_value / 2 # Cost of replacement
                else:
                    reward = -5.0
                ...
                if action.action_type == ActionType.REPLACE:
                    ...
                    business_impact = -10.0 # Shipping cost
        ...
        if reward > 0:
            # Simple sentiment-based bonus
            if observation.sentiment_score < 0.3 and action.action_type != ActionType.RESPOND:
                reward += 3.0
                reason += " | Bonus: Improved sentiment for angry customer."
            
            # Politeness/Length bonus
            if len(action.response_text) > 50:
                reward += 2.0
                reason += " | Bonus: Detailed response."
